namespace Assignment8ex2
{
    public class MathSolutions
    {
        public double GetSum(double x, double y)
        {
            return x + y;
        }
        public double GetProduct(double a, double b)
        {
            return a * b;
        }
        public void GetSmaller(double a, double b)
        {
            if (a < b)
                Console.WriteLine($" {a} is smaller than {b}");
            else if (b < a)
                Console.WriteLine($" {b} is smaller than {a}");
            else
                Console.WriteLine($" {b} is equal to {a}");
        }

        public Func<double, double, double> GetSumFunc => GetSum;
        public Action<double, double> GetSmallerAction => GetSmaller;
    }

    public class Progam
        {
            static void Main(string[] args)
            {
                // create a class object
                MathSolutions answer = new MathSolutions();
                Random r = new Random();
                double num1 = Math.Round(r.NextDouble() * 100);
                double num2 = Math.Round(r.NextDouble() * 100);

                //Console.WriteLine($" {num1} + {num2} = {answer.GetSum(num1, num2)}");
                Console.WriteLine($"{num1} + {num2} = {answer.GetSumFunc(num1, num2)}");


                //Console.WriteLine($" {num1} * {num2} = {answer.GetProduct(num1, num2)}");
                Action<double, double> calcResult = delegate (double x, double y) { Console.WriteLine($"{num1} * {num2} = {num1 * num2}"); };
                calcResult(num1, num2);

                //answer.GetSmaller(num1, num2);
                answer.GetSmallerAction(num1, num2);
                Console.ReadLine();
        
            }
        }
    }